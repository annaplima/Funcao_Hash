...
def verifica_hash(frases_lista: List[str], sha256_lista: List[str], md5_lista: List[str]) -> str:
    for idx in range(len(frases_lista)):
        frase = frases_lista[idx]
        sha256_hash = sha256_lista[idx]
        md5_hash = md5_lista[idx]

        if sha256(frase, sha256_hash):
            print(f"[{idx}] Verifica SHA256")
            print(f"Frase: '{frase}' --> \033[92mSHA256 correto\033[0m")
            print(f"SHA256: '{sha256_hash}'")
        else:
            print(f"[{idx}] Verifica SHA256")
            print(f"Frase: '{frase}' --> \033[91mSHA256 incorreto\033[0m")
            print(f"SHA256: '{sha256_hash}'")

        if MD5(frase, md5_hash):
            print(f"[{idx}] Verifica MD5")
            print(f"Frase: '{frase}' --> \033[92mMD5 correto\033[0m")
            print(f"MD5: '{md5_hash}'\n")
        else:
            print(f"[{idx}] Verifica MD5")
            print(f"Frase: '{frase}' --> \033[91mMD5 incorreto\033[0m")
            print(f"MD5: '{md5_hash}'\n")


def sha256(frase: str, cod: str) -> bool:
    SHA256 = hashlib.sha256(frase.encode('utf-8')).hexdigest()
    return SHA256 == cod


def MD5(frase: str, cod: str) -> bool:
...
def main():
    ...
    verifica_hash(frases_lista, sha256_lista, md5_lista)
